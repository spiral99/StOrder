...
${CHECK_STATUS_URL}  https://ms-standingorder-gateway.consumer-payments.apps.devocp.safaricom.net

*** Test Cases ***
Check Status Test Case   ${msisdn}    ${x-user-auth}    ${x-token-key}
...
Run Each Row As A Single Test Case
    ...
    Test Post Check Status Request    ${msisdn}    ${x-user-auth}    ${x-token-key}
...
Test Post Check Status Request
    ...
    ${body}=    Set Variable    {"ServiceRequest":{"Header":{"RequestRefID":"1691518855893","OperationName":"check-status","SourceSystem":"MiniApp"},"Body":{"Data":[{"Key":"msisdn","Value":"${msisdn}"}]}}}

    Create Session    create_group    ${CHECK_STATUS_URL}

    ${response}  POST On Session    create_group    /api/v1/paymentschedulesdue/check-opt-in-status    data=${body}    headers=${headers}
