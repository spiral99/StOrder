...
${OPT_OUT_URL}  https://ms-standingorder-gateway.consumer-payments.apps.devocp.safaricom.net

*** Test Cases ***
Opt Out Test Case   ${msisdn}    ${x-user-auth}    ${x-token-key}
...
Run Each Row As A Single Test Case
    ...
    Test Post Opt Out Request    ${msisdn}    ${x-user-auth}    ${x-token-key}
...
Test Post Opt Out Request
    ...
    ${body}=    Set Variable    {"ServiceRequest":{"Header":{"RequestRefID":"1691518855893","OperationName":"optout","SourceSystem":"MiniApp"},"Body":{"Data":[{"Key":"msisdn","Value":"${msisdn}"}]}}}

    Create Session    opt_out    ${OPT_OUT_URL}

    ${response}  POST On Session    opt_out    /api/v1/paymentschedulesdue/optout    data=${body}    headers=${headers}
